...
def replace_placeholders_in_docx(template_path, placeholders, output_path):
    ...
    заменяет все вхождения {ключ} -> значение по run'ам (чтобы сохранить формат),
    а затем сохраняет в output_path.
    """
    doc = docx.Document(template_path)

    # --- ЗАМЕНА В ПАРАГРАФАХ (с сохранением формата) ---
    ...
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
                # В каждой ячейке тоже могут быть несколько параграфов
...
def export_to_pdf(docx_path, pdf_path):
    """Открывает docx в Word и экспортирует в PDF."""
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("КАСКО - Автоматизация документов")
    ...
    def load_config(self):
        ...
        if selected_emp_index < len(self.employees):
    ...
    def on_close(self):
        # Запоминаем индекс выбранного сотрудника
    ...
    def get_date_plus_2(self):
        ...
        try:
            ...
            # Если пользователь ввёл дату в неверном формате, вернём пустую строку
            return ""
    ...
    def open_oblozhka(self):
        ...
        temp_docx = os.path.join(CASCO_DOCS_FOLDER, f"temp_oblozhka_{now_str}.docx")
        out_pdf_path = os.path.join(CASCO_PDF_FOLDER, f"Обложка_{now_str}.pdf")

        replace_placeholders_in_docx(TEMPLATE_OBLOZHKA, placeholders, temp_docx)
        try:
            export_to_pdf(temp_docx, out_pdf_path)
    ...
    def open_prodlenie(self):
        """
        Открыть продление (PDF).
        Дата постановления о продлении = дата регистрации + 2 дня.
        То есть создаём новый плейсхолдер {ORDER_DATE_PRODLENIE}.
        """
        ...
        placeholders = self.make_placeholders()
        # Добавляем в placeholders дату продления
        placeholders["{ORDER_DATE_PRODLENIE}"] = self.get_date_plus_2()

        now_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        temp_docx = os.path.join(CASCO_DOCS_FOLDER, f"temp_prodlenie_{now_str}.docx")
        out_pdf_path = os.path.join(CASCO_PDF_FOLDER, f"Продление_{now_str}.pdf")
    ...
    def save_and_open_postanovlenie(self):
        ...
        placeholders = self.make_placeholders()
        now_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        docx_name = f"Постановление_{now_str}.docx"
        pdf_name = f"Постановление_{now_str}.pdf"
    ...
    def save_and_open_soprovod(self):
        """Сохранить и открыть сопроводительный (только PDF)."""
        ...
        placeholders = self.make_placeholders()
        now_str = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        temp_docx = os.path.join(CASCO_DOCS_FOLDER, f"temp_soprov_{now_str}.docx")
        out_pdf_path = os.path.join(CASCO_PDF_FOLDER, f"Сопровод_{now_str}.pdf")

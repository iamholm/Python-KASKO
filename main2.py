...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("КАСКО - Автоматизация документов        15 отдел полиции           v. 1.0")
    ...
    def on_close(self):
        # Сохраняем config и employees (как уже есть)
        ...
        # ---- Удаляем все файлы из self.temp_folder ----
        if os.path.exists(self.temp_folder):
            ...
            # Если хотите удалить и саму папку Temp (пустую):
            try:
    ...
    def open_oblozhka(self):
        ...
        try:
            export_to_pdf(temp_docx, temp_pdf)
            open_file(temp_pdf)
        except Exception as e:
